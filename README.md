...
项目定位为竞赛级可信新闻 AI 作品：核心能力可以在无网络、未配置 AI 密钥的环境中完整运行，适合本地演示、答辩与现场评审。
...
- 可信度实验室：统一说明 60 篇六类别新闻集、四种算法基线、自动指标和人工盲评方法；私有正文不公开。
...
如需主动检索公开来源，在“设置”页面单独填写 Brave Search API Key，先测试连接，再显式保存。联网核验只向搜索服务发送由主体、事件、时间或数值组合而成的短查询，不发送整篇新闻正文。系统只接受 HTTPS 公开地址，并限制查询数量、来源数量、跳转次数、响应大小和总耗时；远程网页全文不会写入 SQLite 历史。
...
| `PUT /api/search-config`            | 保存本机 Brave Search API 配置              |
...
当前 V2.2 开发验收结果：后端 37 项测试通过，前端 8 项测试通过，生产构建和前后端静态检查通过；真实浏览器已复验 1440px、1024px、900px、375px，均无横向溢出。公开来源的真实联网核验需要用户自行配置有效 Brave Key，AI 解读也需要用户主动配置有效 AI Key；项目不会伪造在线来源、模型建议或真实性结论。竞赛发布前仍需完成 60 篇私有新闻标注与 5–8 人盲评，才可填写最终算法结论。
...
- 项目不包含新闻爬虫、链接解析、账号体系、云端同步、批量处理、模型训练或多媒体摘要。
- 竞赛评测的公开仓库只保存来源元数据和汇总指标；正文快照、人工标注和盲评原始评分保留在 `backend/benchmark_private/`，不会提交。
...
本项目为课程实训作品，当前未单独声明开源许可证。公开发布前请补充许可证与使用说明。
